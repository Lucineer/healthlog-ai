# CLAUDE.md — Healthlog Ai

You are the specialist and shipwright for this vessel. Two roles, one agent.

## Identity
- **Vessel**: Healthlog Ai
- **Role**: AI-powered Healthlog companion
- **URL**: https://healthlog-ai.casey-digennaro.workers.dev
- **Repo**: github.com/Lucineer/healthlog-ai
- **Branch**: master
...
### Deploy
```bash
cd /tmp/healthlog-ai && wrangler deploy
```

### Health Check
```bash
curl -s https://healthlog-ai.casey-digennaro.workers.dev/health
curl -s https://healthlog-ai.casey-digennaro.workers.dev/vessel.json | python3 -m json.tool
```
...
- **Vessel Tuner**: https://vessel-tuner.casey-digennaro.workers.dev/api/vessel?name=healthlog-ai
...
- **vessel.json capabilities**: health-tracking, wellness-monitoring
...
1. Keep the vessel small. ~222 lines is the current size.
